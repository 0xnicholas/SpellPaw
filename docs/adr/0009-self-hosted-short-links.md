...
**Consequences**:
- Redirects are handled by Next.js Middleware (no React rendering overhead; < 30ms). Redis caches hot links; miss falls back to PostgreSQL.
- Click events are written via BullMQ fire-and-forget (3 retries, exponential backoff) — the redirect does not block on the Interaction write, but no click is silently lost.
- Hot links (freshly published posts) stay in Redis TTL 24h; after the traffic spike decays, lookups fall through to the DB naturally.
