...
- **Frontier query**: list the map's open children (`gh issue list --state open`, scoped by label), drop any with an open blocker (open issue in the `Blocked by` line) or an assignee; first in map order wins.
